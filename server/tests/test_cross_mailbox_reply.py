...
def test_personal_reply_requires_assignment_and_recorded_send(conn, mailbox, variation, joins):
    ...
    sent, _ = store_raw(
        ...
        mailbox,
        make_raw(
            message_id="<personal@test>",
            from_="cloud@example.test",
    ...
    if variation != "unrecorded":
        conn.execute(
            ...
            "VALUES(?,?,?,?,?)",
            (mailbox, tid, sent, "cloud@example.test", NOW.isoformat()),
        )
