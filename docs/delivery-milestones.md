...
2026-09-25 20:52 CST 当前状态。顶部内容是权威现况；后面的带时间戳记录是历史，不得覆盖当前事实。
...
| M4 生产业务 | 登录、页面、收信发信、模型和交接验收 | 阿里云运行 Aimail main `46b1d63347fc27938865707080658f322977c875`。内部 `/healthz` 200、SQLite 完整性 `ok`、外键错误 0；匿名邮件与登录入口 HTTPS 302。Tailscale 已入网，Spark LiteLLM 网关可达，真实推理需专用 fast 密钥；未发送客户邮件。 |
...
| M6 总交付 | infra 账本、图和实际部署版本一致；附验收证据 | infra PR #232–#235 已合并；生产图记录六台设备、Aimail 当前 SHA/备份/运行服务/timer。仍待第二员工、真实业务页面和 fast 模型验收。 |

## 当前生产事实（2026-09-25 20:52 CST）
