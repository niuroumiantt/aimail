...
2026-09-25 22:43 CST 当前状态。顶部内容是权威现况；后面的带时间戳记录是历史，不得覆盖当前事实。

| 里程碑 | 验收要求 | 当前状态 |
| --- | --- | --- |
| M1 身份与发件 | sales 只收信；各员工使用自己的 SMTP | Larry 的 OIDC 会话和 173 个既存邮件会话已在浏览器验证；sales 收件与个人发件凭据通过协议认证。sales 不在发件名单，自动开发信关闭。第二员工和人工发送验收待完成。 |
...
| M4 生产业务 | 登录、页面、收信发信、模型和交接验收 | 阿里云运行 Aimail main `46b1d63347fc27938865707080658f322977c875`。内部 `/healthz` 200、SQLite 完整性 `ok`、外键错误 0；匿名邮件与登录入口 HTTPS 302。专用 LiteLLM key 仅允许 Spark `fast` 路由，Aimail 结构化合成推理通过，未授权 `brain` 请求返回 403。第二员工、真人页面、业务交接与实际邮件往来仍未验收；未发送客户邮件。 |
| M5 命名职责 | GitHub repo、运行服务/数据路径迁移；确认 OA 邮件代码边界 | GitHub repo、应用包、OCI 镜像、Compose 服务及数据目录已改为 aimail；旧容器停止保留回退、旧路径符号链接保留兼容。OA 的系统通知与 Aimail 客户邮件职责不同；不删除 OA 通知能力。m5 本地 checkout 仍在 `~/code/mail2leads`，当前 `lsof` 实测 uv、python 和 Node 进程仍以该目录为工作目录，且多个本地任务引用此路径；停止或迁移这些使用者前不搬目录。 |
| M6 总交付 | infra 账本、图和实际部署版本一致；附验收证据 | infra PR #232–#239 已合并；生产图记录六台设备、Aimail 当前 SHA/备份/运行服务/timer 与模型调用/限权证据。仍待第二员工、真实业务页面与交接验收。 |

## 当前生产事实（2026-09-25 22:43 CST）
...
- `mail.glocalstorage.cn` 在现有浏览器 OIDC 会话中识别出 Larry，并加载个人邮箱的 173 个邮件会话；这只验收登录和现存数据可见，不代表实时同步、回复、转交或第二员工权限已验收。Aliyun 已加入现有 tailnet，ACL 仅允许访问 Spark LiteLLM HTTPS `:4000`；22:43 CST 已完成 `fast` 路由推理验收，详情见下一条。
...
2. 如果需要测试真实发信，请给出测试收件人及允许发送的精确内容；此前和本次均未发送客户邮件。

第二员工资料或浏览器真人验收未完成时，我会继续推进 OA 邮件引用审计和生产文档收口；不会因等待输入停止其他工作。
