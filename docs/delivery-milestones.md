...
| 里程碑 | 验收要求 | 状态 |
| --- | --- | --- |
| M1 身份与发件 | sales 只收信，各员工使用自己的 SMTP | 多账号、个人回复和收件箱同步代码已合并部署；真实第二员工和真实个人 SMTP 尚未验收 |
| M2 随时交接 | AI 总结、原邮件附件、接手/再转交、接手人续跟进 | 交接及首次联系前潜客分配代码已合并部署；模拟测试通过，真人登录交接待验收 |
| M3 业务入口 | leadsgen→aimail 有回执；sales 来信正确分配；跨邮箱新回复关联并提醒 | leadsgen 只读接口与 aimail 交接代码已部署；真实分配、人员接手和回复提醒待验收 |
| M4 生产业务 | 模型、登录、真实页面、收信发信与交接验收 | `mail.glocalstorage.cn` 运行 aimail 品牌镜像 `53efd32`；健康和匿名 OIDC 边界通过；模型、第二员工、真实 IMAP/SMTP 和交接待验收 |
| M5 命名职责 | aimail 改名、发布/备份引用、OA 重复邮件退役 | 用户界面已改名；GitHub repo、运行服务、数据路径仍叫 `mail2leads`；OA 旧邮件代码审计已合并到 infra，未删除仍有引用的 OA 底层模块 |
| M6 总交付 | infra 账本、图和实际部署版本吻合；附验收证据 | infra PR #217 已合并；aimail 自动发布 CI 与 Aliyun 更新器正在本分支实现，尚未合并或安装 |

## 当前状态（2026-09-25）

PR #22 与 #23 已合并；生产 aimail 容器运行镜像
`mail2leads:aimail-20260925-53efd32`，使用现有 `/srv/mail2leads-data`。SQLite 完整性检查
通过，邮件、附件和会话数量仍为 307/391/230。匿名 `/api/followups` 请求仍由 OIDC 代理
跳转登录。合并和健康检查不代表真人登录、多员工权限或邮件业务已验收。

当前构建将 GitHub Actions 制作为 CI 后置步骤：测试通过才生成完整 SHA Release 和镜像归档；
Aliyun 更新器将按精确 `main` SHA、bot 来源、归档摘要及镜像标签验证，备份 SQLite 后只更新
aimail 服务。该机制尚未合并部署；生产容器当前仍由人工更新。服务器运行更新器需要一次
root 安装；SSH 登录可用，但此会话里的 sudo 要求交互密码。
