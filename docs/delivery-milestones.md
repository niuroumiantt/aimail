...
| M4 生产业务 | 登录、页面、收信发信、模型和交接验收 | 阿里云运行 Aimail main `46b1d63347fc27938865707080658f322977c875`。内部 `/healthz` 200、SQLite 完整性 `ok`、外键错误 0；匿名邮件与登录入口 HTTPS 302。Larry 的 OIDC 会话和邮件列表已由真人浏览器确认。专用 LiteLLM key 仅允许 Spark `fast` 路由，Aimail 结构化合成推理通过，未授权 `brain` 请求返回 403。第二员工、双人业务交接/接手、收发信业务场景仍未验收；未发送客户邮件。 |
...
| M6 总交付 | infra 账本、图和实际部署版本一致；附验收证据 | infra PR #232–#240 已合并；生产图记录六台设备、Aimail 当前 SHA/备份/运行服务/timer 与模型调用/限权证据。仍待第二员工注册与审批、真实业务页面与交接验收。 |

## 当前生产事实（2026-09-25 23:08 CST）
