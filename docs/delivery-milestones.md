...
| M5 命名职责 | GitHub repo、运行服务/数据路径迁移；确认 OA 邮件代码边界 | GitHub repo、应用包、OCI 镜像、Compose 服务及数据目录已改为 aimail；旧容器停止保留回退、旧路径符号链接保留兼容。OA 的系统通知与 Aimail 客户邮件职责不同；不删除 OA 通知能力。m5 本地 checkout 仍在 `~/code/mail2leads`，因闲置 Codex 任务引用该路径，待安全迁移引用后改名。 |
